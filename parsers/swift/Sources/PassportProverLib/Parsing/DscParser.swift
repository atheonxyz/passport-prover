...
public enum DscParser {
    ...
    /// Parse a Certificate from its ASN.1 SEQUENCE node.
    public static func parse(certNode: ASN1Node) throws -> DSC {
        ...
        return DSC(tbs: tbsCert, signatureAlgorithm: certSigAlg, signature: signatureData)
    }
    ...
    // MARK: - BIT STRING Extraction

    /// Extract the content of a BIT STRING, skipping the unused-bits byte.
    ...
    private static func parseTBS(_ node: ASN1Node) throws -> TBSResult {
        ...
        return TBSResult(
            version: version, serialNumber: serialNumber, signatureAlgorithm: tbsSigAlg,
            issuer: issuer, notBefore: notBefore, notAfter: notAfter, subject: subject, spki: spki
        )
    }
    ...
    private static func formatX500Name(_ node: ASN1Node) throws -> String {
        ...
        for rdnSet in rdnSeqs {
            for atv in try ASN1.parseSequence(rdnSet.data) {
